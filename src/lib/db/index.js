...
export async function exportDb() {
  ...
  const out = {
    ...
    visionBridgeProfiles: db.all(`SELECT * FROM visionBridgeProfiles`).map((r) => ({ id: r.id, name: r.name, enabled: r.enabled !== 0, config: parseJson(r.config, {}), createdAt: r.createdAt, updatedAt: r.updatedAt })),
    modelAliases: {},
    customModels: [],
    mitmAlias: {},
    pricing: {},
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    db.run(`DELETE FROM visionBridgeProfiles`);
    db.run(`DELETE FROM attachmentDescriptions`);
    ...
    for (const profile of payload.visionBridgeProfiles || []) {
      db.run(
        `INSERT OR REPLACE INTO visionBridgeProfiles(id, name, enabled, config, createdAt, updatedAt) VALUES(?, ?, ?, ?, ?, ?)`,
        [profile.id, profile.name, profile.enabled === false ? 0 : 1, stringifyJson(profile.config || {}), profile.createdAt || new Date().toISOString(), profile.updatedAt || new Date().toISOString()]
      );
    }
